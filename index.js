import{a as v,S as M,i as k}from"./assets/vendor-C59OI8Mo.js";(function(){const s=document.createElement("link").relList;if(s&&s.supports&&s.supports("modulepreload"))return;for(const n of document.querySelectorAll('link[rel="modulepreload"]'))i(n);new MutationObserver(n=>{for(const o of n)if(o.type==="childList")for(const r of o.addedNodes)r.tagName==="LINK"&&r.rel==="modulepreload"&&i(r)}).observe(document,{childList:!0,subtree:!0});function t(n){const o={};return n.integrity&&(o.integrity=n.integrity),n.referrerPolicy&&(o.referrerPolicy=n.referrerPolicy),n.crossOrigin==="use-credentials"?o.credentials="include":n.crossOrigin==="anonymous"?o.credentials="omit":o.credentials="same-origin",o}function i(n){if(n.ep)return;n.ep=!0;const o=t(n);fetch(n.href,o)}})();const E=[{question:"Я мрію про пухнастика! Що мені потрібно зробити, щоб забрати хвостика додому?",answer:`Це чудово, що ви готові подарувати дім одному з наших підопічних! Ми дуже раді будемо вам у цьому допомогти. Процес «усиновлення» у нас простий та зрозумілий:
  ...
  `).join(""),e.addEventListener("click",s=>{const t=s.target.closest(".accordion-header");if(!t)return;const i=t.parentElement,n=i.classList.contains("active");document.querySelectorAll(".accordion-item").forEach(o=>{o.classList.remove("active")}),n||i.classList.add("active")}))};document.addEventListener("DOMContentLoaded",P);const b={tailsFilters:document.querySelector(".tails-filters"),tailsProducts:document.querySelector(".tails-products"),tailsBtn:document.querySelector(".tails-btn")},B="https://paw-hut.b.goit.study/api/",L={categories:"categories",animals:"animals"};v.defaults.baseURL=B;const T=async()=>{const{data:e}=await v(L.categories);return e},A=async()=>{const{data:e}=await v(L.animals);return e};function C(e){const s=e.map(({_id:t,name:i})=>`
<li class="category-item" data-id="${t}">
    <button type="button" class="category-btn">${i}</button>
</li>`).join("");b.tailsFilters.innerHTML=s}function O(e){const s=e.animals.map(({_id:t,image:i,name:n,categories:o,age:r,gender:c,shortDescription:u,description:S,species:$})=>`
    ...
    </li>`).join("");b.tailsProducts.insertAdjacentHTML("beforeend",s)}function F(e){return`
    ...
    </div>`}(()=>{const e={openModalBtn:document.querySelector("[data-burger-open]"),closeModalBtn:document.querySelector("[data-burger-close]"),overlay:document.querySelector("[data-burger-overlay]"),modal:document.querySelector("[data-burger]")};e.openModalBtn.addEventListener("click",s),e.closeModalBtn.addEventListener("click",t),e.overlay.addEventListener("click",t),document.addEventListener("keydown",n=>{n.key==="Escape"&&e.modal.classList.contains("is-open")&&t()});function s(){e.modal.classList.add("is-open"),document.body.style.overflow="hidden"}function t(){e.modal.classList.remove("is-open"),document.body.style.overflow=""}e.modal.querySelectorAll('a[href*="#"]').forEach(n=>{n.addEventListener("click",t)})})();function N(){const e=document.querySelector(".category-btn");e&&e.classList.add("accent")}j();I();async function j(){try{const e=await T();C([{name:"Всі",_id:"all"},...e]),N()}catch(e){console.log(e)}}let f=[];async function I(){try{const e=await A();f=e.animals,O(e)}catch(e){console.log(e)}}const a=document.querySelector('[data-modal="order"]'),l=a==null?void 0:a.querySelector(".order-form"),m=a==null?void 0:a.querySelector(".js-order-close"),D=document.querySelectorAll("[data-order-open]");function V(){a&&(a.classList.remove("is-hidden"),document.body.style.overflow="hidden",window.addEventListener("keydown",w),a.addEventListener("click",q))}function y(){a&&(a.classList.add("is-hidden"),document.body.style.overflow="",window.removeEventListener("keydown",w),a.removeEventListener("click",q))}function w(e){e.key==="Escape"&&y()}function q(e){e.target===a&&y()}D.forEach(e=>e.addEventListener("click",V));m==null||m.addEventListener("click",y);l==null||l.addEventListener("submit",U);function h(e,s){const t=e.closest(".form-field"),i=t==null?void 0:t.querySelector(".error-text");e.classList.add("error"),i&&(i.textContent=s,i.classList.add("visible"))}function d(e){const s=e.closest(".form-field"),t=s==null?void 0:s.querySelector(".error-text");e.classList.remove("error"),t&&(t.textContent="",t.classList.remove("visible"))}function g(e,s){return e.value.trim()?(d(e),!0):(h(e,s),!1)}["input","change"].forEach(e=>{l==null||l.addEventListener(e,s=>{const t=s.target;t.matches("input, textarea")&&t.classList.contains("error")&&t.value.trim()&&d(t)})});function R(e){const s=e.value.trim();return/^380\d{9}$/.test(s)?(d(e),!0):(h(e,"Номер телефону має містити 12 цифр і починатися з 380"),!1)}function z(e){const s=e.value.trim();return/^[A-Za-zА-Яа-яІіЇїЄєҐґ\s'-]{3,}$/.test(s)?(d(e),!0):(h(e,"Ім'я має містити мінімум 3 літери"),!1)}async function U(e){e.preventDefault();const{name:s,phone:t,comment:i}=e.target.elements,o=g(s,"Будь ласка введіть ім'я")?z(s):!1,c=g(t,"Будь ласка введіть номер телефону")?R(t):!1;if(!(!o||!c)){M.fire({icon:"warning",title:"Оберіть тваринку",text:"Спочатку відкрийте картку тварини та натисніть “Взяти друга”."});return}}const p=document.getElementById("mw");document.addEventListener("click",async e=>{const s=e.target,t=s.closest("[data-modal-open]"),i=s.closest("[data-modal-close]"),n=s.classList.contains("backdrop");if(console.log(f),t){const o=t.dataset.id,r=t.textContent;t.disabled=!0,t.textContent="Завантаження...";try{const c=f.find(u=>u._id===o);if(!c)throw new Error("Тваринку не знайдено");p.innerHTML=F(c),document.body.style.overflow="hidden"}catch{k.error({title:"Помилка",message:"Щось пішло не так",position:"topRight"})}finally{t&&(t.disabled=!1,t.textContent=r)}return}if(i||n){const o=document.querySelector(".animal-modal"),r=document.querySelector(".backdrop");o==null||o.classList.add("exit-up"),r==null||r.style.setProperty("opacity","0"),setTimeout(()=>{p.innerHTML="",document.body.style.overflow=""},400)}s.closest("[data-modal-next]")&&(document.querySelector(".animal-modal").classList.add("exit-up"),setTimeout(()=>{p.innerHTML=H()},400))});
